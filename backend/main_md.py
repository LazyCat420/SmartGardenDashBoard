...
from llm_service import (
    load_llm_settings, save_llm_settings,
    DEFAULT_LMSTUDIO_URL, DEFAULT_MODEL_NAME
)
...
LMSTUDIO_URL = _llm_settings.get("url", DEFAULT_LMSTUDIO_URL)
MODEL_NAME = _llm_settings.get("model", DEFAULT_MODEL_NAME)
...
def llm_status():
    """Check if LMStudio is running and responding"""
    ...
    base_url = url.rsplit('/v1/', 1)[0] if '/v1/' in url else url.rsplit('/chat/completions', 1)[0]
    models_url = f"{base_url}/v1/models"
    
    try:
        # First check if LMStudio is reachable
        ...
        if test_response.status_code == 200:
            ...
            return jsonify({
                ...
                'url': url,
                'model': model,
                'message': 'LMStudio is connected and responding',
            ...
            return jsonify({
                ...
                'url': url,
                'model': model,
                'message': f'Model test failed with status {test_response.status_code}',
                'error_details': test_response.text[:200] if test_response.text else None
            })
        ...
        return jsonify({
            ...
            'url': url,
            'model': model,
            'message': f'Cannot connect to LMStudio at {base_url}'
        })
        ...
        return jsonify({
            ...
            'url': url,
            'model': model,
            'message': 'Connection timed out - is LMStudio running?'
        })
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify({
                "success": True,
                "models": model_list,
                "current": settings.get("model", DEFAULT_MODEL_NAME)
            })
        else:
            return jsonify({
                "success": False,
                "models": [],
                "message": f"Failed to fetch models: status {response.status_code}"
            })
    except requests.exceptions.ConnectionError:
        return jsonify({
            "success": False,
            "models": [],
            "message": "Cannot connect to LMStudio - is it running?"
        })
    except Exception as e:
        return jsonify({
            "success": False,
...
def test_tool_calling():
    ...
    try:
        response = requests.post(
            ...
            json={
                "model": model,
                "messages": [{"role": "user", "content": "What is 2+2? Use a function to calculate."}],
                "tools": [{
                    "type": "function",
                    "function": {
                        "name": "calculate",
        ...
        if response.status_code == 200:
            ...
            if tool_calls:
                return jsonify({
                    "supports_tools": True,
                    "message": f"Model supports tool calling! Found {len(tool_calls)} tool call(s).",
                    "tool_calls": tool_calls
                })
            else:
                return jsonify({
                    "supports_tools": False,
                    "message": "Model did not use tool calling",
                    "hint": "Try using a model that supports function calling like Qwen, Llama-3.1+, or Mistral",
                    "model_response": content[:200] if content else None
                })
        else:
            return jsonify({
                "supports_tools": False,
...
def get_leaderboard():
    ...
    # Get active plants
    plants_list = list_plants(status='active')
    
    # Filter by specific plant IDs if provided
    if plant_ids:
        try:
            ids = [id.strip() for id in plant_ids.split(',') if id.strip()]
    ...
    if category and category != 'all':
        plants_list = [p for p in plants_list if (p.get('name') or '').lower() == category.lower()]
    ...
    for plant in plants_list:
        ...
        plant_data = {
            'id': plant_id,
            'name': plant_name,
            'display_name': plant.get('display_name') or f"{plant_name} {plant.get('variety', '')} #{plant.get('instance', 1)}".strip(),
            'variety': plant.get('variety'),
            'category': plant_name.lower(),  # Use plant name as category
        }
        
        # Get growth logs for this plant
        growth_logs = get_growth_logs(plant_id) or []
        
        # Calculate growth rate (cm/day)
        if len(growth_logs) >= 2:
            sorted_logs = sorted(growth_logs, key=lambda x: x.get('date', ''))
            first_log = sorted_logs[0]
            last_log = sorted_logs[-1]
            
            try:
                first_date = datetime.fromisoformat(first_log.get('date', '').replace('Z', '+00:00'))
                last_date = datetime.fromisoformat(last_log.get('date', '').replace('Z', '+00:00'))
                days = (last_date - first_date).days or 1
                
                first_height = first_log.get('height_cm') or 0
                last_height = last_log.get('height_cm') or 0
        ...
        health_ratings = [log.get('health_rating') for log in growth_logs if log.get('health_rating')]
        ...
        plant_data['harvests'] = len(harvests)
        
        # Get latest height
        if growth_logs:
            sorted_logs = sorted(growth_logs, key=lambda x: x.get('date', ''), reverse=True)
            plant_data['latest_height'] = sorted_logs[0].get('height_cm') or 0
        else:
            plant_data['latest_height'] = 0
    ...
    # Get unique categories from all active plants
    all_plants = list_plants(status='active')
    categories = list(set((p.get('name') or '').lower() for p in all_plants if p.get('name')))
    categories.sort()
